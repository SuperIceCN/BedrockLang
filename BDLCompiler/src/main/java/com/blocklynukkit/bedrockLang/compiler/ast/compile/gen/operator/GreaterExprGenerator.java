...
public final class GreaterExprGenerator implements ExprCodeGenerator, BoxHelper {
    private final GreaterExpr expr;

    public GreaterExprGenerator(GreaterExpr expr) {
        this.expr = expr;
    }
    ...
    public ValueType generate(Unit unit) {
        ...
        if ((isNumberType(leftValueType) && isNumberType(rightValueType)) ||
            ...
            if ((isBoxType(leftValueType) && isNumberType(rightValueType)) ||
                ...
                mv.visitJumpInsn(IFGT, trueLabel); // 大于零
